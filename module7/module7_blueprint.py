...
def triangulate_and_estimate(p1_L, p1_R, p2_L, p2_R, reference_depth_cm):
    ...
    if abs(Z1_scaled) < 1e-6:  # Check for near-zero depth
...
@module7_bp.route('/')
def index():
    """Main page for Project Zeta - Stereo Distance Calculator."""
    return render_template('module7/index_stereo.html')


@module7_bp.route('/calculate', methods=['POST'])
def calculate_distance():
    """API endpoint to calculate 3D distance from stereo image pairs."""
    data = request.json
